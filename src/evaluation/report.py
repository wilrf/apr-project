...
class ReportGenerator:
    ...
    def _generate_comparison(self) -> Dict[str, Any]:
        ...
        comparison = {"metric_rankings": {}, "pairwise": {}, "agreement": {}, "correlation": {}}
    ...
    def _generate_disagreement_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "agreement_matrix": analyzer.get_agreement_matrix().to_dict(orient="records"),
            ...
            if s["comparison"].get("pairwise"):
                ...
                for pair, stats in s["comparison"]["pairwise"].items():
                    ...
                    if "prediction_correlation" in stats:
                        lines.append(f"  - Prediction Correlation: {stats['prediction_correlation']:.3f}")
                    if "agreement_rate" in stats:
                        lines.append(f"  - Classification Agreement: {stats['agreement_rate']:.1%}")
                    lines.append("")

            if s["comparison"].get("agreement", {}).get("all_models"):
                lines.append(f"**All Models Agree:** {s['comparison']['agreement']['all_models']:.1%} of predictions")
                lines.append("")
